...
function findAvailableRoom(): string | null {
    for (const room in ROOMS) {
        if (
            ROOMS[room].players.length < MAX_PLAYERS &&
...
async function fetchRandomProblem(): Promise<number> {
    try {
        const response = await axios.get<RandomProblemType>(
            BASE_URL_API + "/api/problems/randomId"
        );
    ...
    try {
        ...
        for (const room of roomKeys) {
            ...
            if (index !== -1) {
                ...
                if (socketInstance) {
                    io.to(removed.socketId).emit(
                        "forceDisconnect",
                        "You have been replaced by a new connection."
                    );
...
io.on("connection", (socket) => {
    ...
    socket.on("updatePercentage", (user: IUserPercentage) => {
        ...
        io.to(user.roomKey).emit(
            "receivedPercentageUpdatePlayer",
            ROOMS[user.roomKey].players
        );
        console.log(
            `Updated percentage for ${user.userId} in ${user.roomKey}: ${user.percentage}`
        );
    });
                ...
                if (ROOMS[room]) {
                    ...
                    if (playerIndex !== -1) {
                        ...
                        if (ROOMS[room].players.length === 0) {
                            delete ROOMS[room];
                            console.log(
                                `Room ${room} deleted because it's empty.`
                            );
                        } else {
                            io.to(room).emit("roomUpdate", ROOMS[room]);
                        }
    ...
    socket.on("message", (msg: string) => {
        try {
            const room = Object.keys(ROOMS).find(
                (r) =>
                    ROOMS[r] &&
                    ROOMS[r].players.some(
                        (detail) => detail.socketId === socket.id
                    )
            );
